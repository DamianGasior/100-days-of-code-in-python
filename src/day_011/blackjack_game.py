...
def card_lottery():
    ...
    return choose_random_card,updated_deck_of_cards
...
def first_two_cards(users_answer):
    ...
    for _ in range (2):   # ciekawa iteracja przez petle, nie podajesz 'i' ale dajesz tylko '_'
        if users_answer=='y':
            ...
            choose_random_card ,updated_deck_of_cards=card_lottery()
...
def sum_of_players_card(card_score,players_total_score):
    
    players_total_score+=card_score
    #return f'Your cards : {players_deck_of_cards}, current score {players_total_score}'
    return players_total_score


def first_two_players_card(players_deck_of_cards):
    ...
    # print(f'Your cards : {players_deck_of_cards}, current score {players_total_score}')
        players_total_score=sum_of_players_card(card_score,players_total_score)
    print(f'Your cards : {players_deck_of_cards}, current score {players_total_score}')
    return players_total_score
...
def round_comparison(dealers_total_score,players_total_score,players_deck_of_cards):
    if players_total_score ==21 and len(players_deck_of_cards)==2: # check for the first round, if maybe the player did won
         return f'Player has won with score :{players_total_score}'
...
def next_car_round(continue_game_question):
    if continue_game_question=='y':
        players_deck_of_cards
        choose_random_card=card_lottery()
        players_deck_of_cards.append(choose_random_card)


     #   take the random card from the deck
      #  compare after each card is taken, the results in round_comparison()





next_car_round(continue_game_question,players_deck_of_cards,players_total_score)